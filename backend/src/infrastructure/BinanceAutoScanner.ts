import { PairTuple, TriangularPairs } from "../application/TriangularPairs";
import { Currency } from "../domain/valueObjects/Currency";
import { Pair } from "../domain/valueObjects/Pair";

export class BinanceAutoScanner {
	public async scanTriangles(
		baseQuoteStr = "USDT",
		finalQuoteStr = "BRL",
	): Promise<TriangularPairs[]> {
		...
		try {
			...
			for (const baseStr of brlBases) {
				...
				if (usdtBases.has(baseStr)) {
					...
					if (
						...
						console.log(
							`🚫 AutoScanner: Banned Stablecoin ${baseStr} (Dynamic Peg Detection - Price: ${priceUsdt})`,
						);
						continue;
					}
			...
			console.log(
				`📡 AutoScanner: Found ${triangles.length} active triangular arbitrage routes for ${baseQuoteStr} -> ${finalQuoteStr}.`,
			);
		} catch (e) {
