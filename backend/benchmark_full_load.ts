...
async function runBenchmark() {
    ...
    for (let i = 0; i < 1000; i++) {
        for (const triangle of activeTriangles) {
            await arbitrageCycle.evaluateAndExecute(triangle, feeFetcher, mathEngine, usableAmount, minProfit, true, stateManager);
        }
    }
    ...
    console.log(`[BENCHMARK] Time to process ALL 17 Triangles sequentially on a single tick: ${timePerFullLoopMs.toFixed(5)} ms`);
    console.log(`[BENCHMARK] Time to process 1 Triangle individually: ${timePerTriangleMs.toFixed(5)} ms`);
