...
class ApiCredentials {
	...
	constructor(
		...
		if (!this.hasKeys) {
			console.warn(
				"⚠️ Binance API Key/Secret missing. All fees will use default 0.1%.",
			);
		}
	}
...
export class BinanceFeeFetcher implements FeeFetcher {
	...
	public async preloadFees(_pairs: Pair[]): Promise<void> {
		...
		try {
			...
			if (!response.ok) {
				console.warn(
					`⚠️ Bulk fee fetch failed: HTTP ${response.status}. Using default fees.`,
				);
				return;
			}
