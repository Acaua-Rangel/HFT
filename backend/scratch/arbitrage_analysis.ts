...
async function analyze() {
	...
	console.log(
		"Comparação de Modelos de Arbitragem (Considerando Taxas com BNB Discount 0.075% por perna):",
	);
	...
	for (const alt of testCoins) {
		...
		try {
			...
			if (usdtBrl && altUsdt && altBrl) {
				...
				console.log(
					`[Opção 1] Rota 3-Pernas Base BRL (BRL->USDT->${alt}->BRL): Lucro de R$ ${route1Profit.toFixed(4)} (Bruto: R$ ${grossBrl.toFixed(4)})`,
				);
			}
			...
			if (btcUsdt && altBtc && altUsdt) {
				...
				const usdtBrlPrice = usdtBrl?.bidPrice || 0;
				const route2ProfitInBrl = route2Profit * usdtBrlPrice;
				const grossUsdtInBrl = grossUsdt * usdtBrlPrice;

				console.log(
					`[Opção 2] Rota 3-Pernas Base USDT (USDT->BTC->${alt}->USDT): Lucro de R$ ${route2ProfitInBrl.toFixed(4)} (Bruto: R$ ${grossUsdtInBrl.toFixed(4)})`,
				);
			}
			...
			if (usdtBrl && btcUsdt && altBtc && altBrl) {
				...
				console.log(
					`[Opção 3] Rota 4-Pernas Base BRL (BRL->USDT->BTC->${alt}->BRL): Lucro de R$ ${route3Profit.toFixed(4)} (Bruto: R$ ${grossBrl.toFixed(4)})`,
				);
			}
			...
			if (usdtBrl && altUsdt && altBrl) {
				...
				console.log(
					`[Opção 4] Rota 3-Pernas Base USDT via BRL (USDT->${alt}->BRL->USDT): Lucro de R$ ${route4ProfitInBrl.toFixed(4)} (Bruto: R$ ${grossUsdtInBrl.toFixed(4)})`,
				);
			}
		} catch (_e) {
