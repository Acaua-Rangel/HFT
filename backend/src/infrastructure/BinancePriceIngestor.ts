import type { PriceIngestor } from "../domain/interfaces/PriceIngestor";
...
class IngestorState {
	...
	private flushPending(): void {
		...
		console.log(
			`📡 Subscribed to ${this.pendingStreams.length} streams: ${this.pendingStreams.join(", ")}`,
		);
		this.pendingStreams = [];
	}
...
export class BinancePriceIngestor implements PriceIngestor {
	...
	private processDepthStream(payload: any): void {
		if (!payload.stream || !payload.data?.asks || !payload.data.bids) {
			return;
		}
		...
		if (this.tickCount === 1 || this.tickCount % 1000 === 0) {
			...
			console.log(
				`📈 Tick [${this.tickCount}]: ${symbol} | Top Ask: ${topAsk} | Top Bid: ${topBid}`,
			);
		}
