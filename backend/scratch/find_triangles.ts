async function run() {
	...
	console.log(
		`Found ${validTriangles.length} valid triangular paths starting with BRL.`,
	);
	...
	const obscureTriangles = validTriangles.filter((t) =>
		obscureAssets.includes(t.asset),
	);

	console.log("\nObscure Triangles:");
	obscureTriangles.forEach((t) =>
		console.log(`${t.leg1} -> ${t.leg2} -> ${t.leg3} (${t.asset}/${t.quote})`),
	);
}
