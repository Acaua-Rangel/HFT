  ...
  it("should apply BNB discount rules dynamically based on asset price", () => {
    ...
    const mockStateManager = {
      retrieveOrderBook: mock((pair: Pair) => {
        ...
        if (sym === "BNBUSDT") {
           return { getLatest: () => new Tick(pair, [{price: new Amount(600), qty: new Amount(1)}], []) };
        }
        
        // Let's pretend BTCUSDT is $60,000 (enough to pay fee)
        if (sym === "BTCUSDT") {
           return { getLatest: () => new Tick(pair, [{price: new Amount(60000), qty: new Amount(1)}], []) };
        }
    ...
    let passedFees: any[] = [];
    const mockEvaluator = {
      evaluate: mock((pairs: any, f1: any, f2: any, f3: any) => {
        passedFees = [f1, f2, f3];
