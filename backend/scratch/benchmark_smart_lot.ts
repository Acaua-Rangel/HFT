...
async function runBenchmark() {
	...
	console.log(
		`[BENCHMARK] Smart Lot Scanner processando ${activeTriangles.length} triângulos × ~${lotSizes.length} lotes:`,
	);
	console.log(
		`⏱️ Tempo médio interno para analisar todas combinações num único tick: ${timePerFullLoopMs.toFixed(5)} ms`,
	);
	console.log(
		`⏱️ Se a API da AWS tem 4.00 ms, essa conta ocupa apenas ${((timePerFullLoopMs / 4) * 100).toFixed(2)}% do tempo da rede.`,
	);
}
