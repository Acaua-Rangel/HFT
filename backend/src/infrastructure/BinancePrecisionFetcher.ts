...
export class BinancePrecisionFetcher implements PrecisionFetcher {
	...
	public async preloadPrecisions(): Promise<void> {
		try {
			...
			if (response.ok) {
				...
				console.log(
					`✅ Preloaded precision filters for ${this.quantityPrecisionCache.size} symbols from Binance API.`,
				);
			} else {
				console.warn(
					`⚠️ Failed to preload exchangeInfo: HTTP ${response.status}`,
				);
			}
		} catch (e) {
	...
	public getQuantityDecimals(symbol: string): number {
		return (
			this.quantityPrecisionCache.get(symbol.toUpperCase()) ??
